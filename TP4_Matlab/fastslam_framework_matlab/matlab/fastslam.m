...
%for t = 1:50
    fprintf('timestep = %d\n', t);
    fprintf('%.3f  ;  %.3f  ; %.1f  ;\n',particles(1).pose(2),particles(1).pose(2), particles(1).pose(3)*180/pi)
...
% relative percentage error of every landmakr for the particle with the
% biggest weight
rpe = zeros(9,2);

for i = 1:9
    rpe(i,1) = abs(particles(bestParticleIdx).landmarks(i).mu(1) - landmarks(i).x) / landmarks(i).x ;
    rpe(i,2) = abs(particles(bestParticleIdx).landmarks(i).mu(2) - landmarks(i).y) / landmarks(i).y ;
end
rpe = rpe  * 100;